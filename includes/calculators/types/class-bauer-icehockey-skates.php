...
class BauerIcehockeySkates extends BorahhCalculatorBase {
    ...
    protected function widthOptions($options) {
        ...
        ?>
        <div>
            <input class="peer" type="radio" name="foot-width" id="<?php echo 'width_' . $option; ?>" x-ref="<?php echo 'width_' . $option; ?>" value="<?php echo $index; ?>" <?php if ($index === 0) echo "checked" ;?>/>
            <div class="img_option_wraper">
                <img src="<?php echo BORAHH_HOCKEYSHOP_IMG_URL . 'foot-width-' . $option . '-inactive.webp'; ?>" alt="" />
                <img class="img_active" :class="$refs.<?php echo 'width_' . $option; ?>.checked() ? 'opacity-100' : 'opacity-0'" src="<?php echo BORAHH_HOCKEYSHOP_IMG_URL . 'foot-width-' . $option . '-active.webp'; ?>" alt="" />
            </div>
