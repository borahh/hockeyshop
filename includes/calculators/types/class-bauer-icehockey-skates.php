...
class BauerIcehockeySkates extends BorahhCalculatorBase {
    ...
    protected function widthOptions($options) {
        ...
        ?>
        <div class="h-[350px]">
            <input class="peer" type="radio" name="foot-width" id="<?php echo 'width_' . $option; ?>" value="<?php echo $index; ?>" <?php if ($index === 0) echo "checked" ;?>/>        
            <img class="peer-checked:opacity-0 absolute top-0 w-[27%]" src="<?php echo BORAHH_HOCKEYSHOP_IMG_URL . 'foot-width-' . $option . '-inactive.webp'; ?>" alt="" />
            <img class="absolute top-0 w-[27%] opacity-0 peer-checked:opacity-100" src="<?php echo BORAHH_HOCKEYSHOP_IMG_URL . 'foot-width-' . $option . '-active.webp'; ?>" alt="" />
            <label for="<?php echo 'width_' . $option; ?>" class="peer-checked:bg-accent peer-checked:text-white absolute top-[250px]"> 
